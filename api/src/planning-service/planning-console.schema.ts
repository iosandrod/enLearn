...
import { PLANNING_CONSOLE_RESTORED_SCHEMA } from './planning-console-restored.schema';
...
function grid(
  ...
  return {
    ...
    ...(options.tableType ? { tableType: options.tableType } : {}),
    clientFilter: false,
    schema: {
      grid: {
        border: true,
...
export function buildPlanningConsolePageSchema(): LowCodePageSchema {
  // Keep the recovered designer layout as the canonical source. The old builder
  // below is retained temporarily for history, but must not be used to regenerate
  // the page because it places the demand grid inside the orders tab.
  return PLANNING_CONSOLE_RESTORED_SCHEMA;

  /* istanbul ignore next -- retained historical builder */
  return {
    schemaVersion: 1,
