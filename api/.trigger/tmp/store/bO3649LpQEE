...
// src/workflow/common/postgres-resilience.ts
init_esm();
var TRANSIENT_POSTGRES_CODES = /* @__PURE__ */ new Set([
  "08000",
  "08001",
  "08003",
  "08004",
...
var TRANSIENT_POSTGRES_MESSAGES = [
...
__name(isTransientPostgresError, "isTransientPostgresError");
async function retryTransientPostgresOperation(operation, options = {}) {
...
__name(retryTransientPostgresOperation, "retryTransientPostgresOperation");
...
async function runLocalNotificationDispatchTask(payload) {
  ...
  const eventRows = await upsertRestRows(
    "notification_events",
  ...
  const messages = await upsertRestRows(
...
__name(runLocalNotificationDispatchTask, "runLocalNotificationDispatchTask");
...
async function upsertRestRows(table, onConflict, rows) {
  if (!rows.length) return [];
  const env = getWorkflowEnv();
  const supabaseUrl = env.SUPABASE_URL ?? env.NEXT_PUBLIC_SUPABASE_URL ?? env.SUPABASE_PROJECT_URL;
  const supabaseKey = env.SUPABASE_SERVICE_ROLE_KEY ?? env.SUPABASE_ANON_KEY ?? env.NEXT_PUBLIC_SUPABASE_ANON_KEY ?? env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY;
...
__name(upsertRestRows, "upsertRestRows");
function localInboxTemplate(eventType) {
...
__name(localInboxTemplate, "localInboxTemplate");
...
//# sourceMappingURL=chunk-P2IJLWLJ.mjs.map
