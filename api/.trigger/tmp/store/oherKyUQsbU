...
// src/workflow/trigger/trigger-credentials.service.ts
init_esm();
...
var DEFAULT_ADMIN_EMAIL = "engine@local.dev";
var DEFAULT_ADMIN_NAME = "enLearn Trigger Engine";
var DEFAULT_PAT_NAME = "enlearn-engine-runtime";
var DEFAULT_PROJECT_NAME = "enlearn-workflow-local";
var WORKFLOW_TASK_IDS = [
  "workflow.instance.run",
  "workflow.job.run",
  "notification.dispatch"
];
var PAT_ALPHABET = "123456789abcdefghijkmnopqrstuvwxyz";
var ID_ALPHABET = "1234567890abcdefghijklmnopqrstuvwxyz";
var TRIGGER_CREDENTIALS_SERVICE_OPTIONS = Symbol("TRIGGER_CREDENTIALS_SERVICE_OPTIONS");
var TriggerCredentialsService = class {
  constructor(options) {
    this.logger = new import_common.Logger(TriggerCredentialsService.name);
    const resolvedOptions = options ?? {};
    this.config = {
      ...resolveCredentialsConfig(),
      ...resolvedOptions.cacheTtlMs === void 0 ? {} : { cacheTtlMs: resolvedOptions.cacheTtlMs }
    };
  ...
  async getCredentials(forceRefresh = false) {
    ...
    this.refreshPromise = (this.loadCredentialsOverride ?? (() => this.loadCredentials()))().then((value) => {
      this.cache = {
        value,
      ...
      if (!forceRefresh && staleCredentials) {
        this.logger.warn(
          `Unable to refresh Trigger.dev credentials; using stale in-memory credentials for 30 seconds: ${error instanceof Error ? error.message : String(error)}`
        );
        this.cache = {
          value: staleCredentials,
          expiresAt: this.now() + Math.min(this.config.cacheTtlMs, 3e4)
        };
  ...
  async configureSdk(forceRefresh = false) {
  ...
  async loadCredentials() {
    ...
    try {
      ...
      this.logger.log(
        `Loaded Trigger.dev credentials from database for ${environment.projectName}/${environment.environmentSlug}; selection=${selection}, cacheTtlMs=${this.config.cacheTtlMs}`
      );
      return {
        accessToken,
  ...
  missingInfrastructure() {
    return [
      this.config.databaseUrl ? void 0 : "TRIGGER_DATABASE_URL",
      this.config.encryptionKey ? void 0 : "TRIGGER_ENCRYPTION_KEY"
    ].filter((value) => Boolean(value));
  }
  async resolveEnvironment(client, admin) {
  ...
  async createProjectEnvironment(client, admin) {
  ...
  async resolveAdminOrganization(client, adminId) {
  ...
  async createProject(client, organizationId) {
    for (let attempt = 0; attempt < 100; attempt += 1) {
  ...
  async createEnvironment(client, options) {
  ...
  async resolveAdminToken(client, userId) {
    const result = await client.query(
    ...
    if (result.rows.length > 0) {
      ...
      throw new Error(
        "Active Trigger.dev admin tokens exist but cannot be decrypted. TRIGGER_ENCRYPTION_KEY must match the Trigger.dev webapp ENCRYPTION_KEY."
      );
    }
    return this.createAdminToken(client, userId);
  }
  async createAdminToken(client, userId) {
...
__name(TriggerCredentialsService, "TriggerCredentialsService");
TriggerCredentialsService = __decorateClass([
  (0, import_common.Injectable)(),
  __decorateParam(0, (0, import_common.Optional)()),
  __decorateParam(0, (0, import_common.Inject)(TRIGGER_CREDENTIALS_SERVICE_OPTIONS))
...
function resolveCredentialsConfig() {
  ...
  const parsedDatabase = rawDatabaseUrl ? parseDatabaseUrl(rawDatabaseUrl) : void 0;
  const environment = normalizeEnvironment(env.TRIGGER_ENVIRONMENT);
  const encryptionKey = normalizedValue(env.TRIGGER_ENCRYPTION_KEY) ?? triggerFileEnv.ENCRYPTION_KEY;
...
__name(resolveCredentialsConfig, "resolveCredentialsConfig");
function resolveTriggerEnvFile(explicitPath) {
  if (explicitPath) {
...
__name(resolveTriggerEnvFile, "resolveTriggerEnvFile");
...
__name(parseDatabaseUrl, "parseDatabaseUrl");
...
__name(setSearchPath, "setSearchPath");
...
__name(normalizeEnvironment, "normalizeEnvironment");
...
//# sourceMappingURL=chunk-L4EZXOEE.mjs.map
