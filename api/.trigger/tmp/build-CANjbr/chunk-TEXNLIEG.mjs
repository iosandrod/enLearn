...
} from "./chunk-S46HXNYX.mjs";
import {
  __name,
  init_esm
} from "./chunk-VDUEJNM7.mjs";

// src/workflow/runtime/runtime.engine.types.ts
init_esm();
var WORKFLOW_RUNTIME_STORE = Symbol("WORKFLOW_RUNTIME_STORE");
var WORKFLOW_INSTANCE_TASK_ID = "workflow.instance.run";
...
function canActorSeeTask(task, candidates, actor) {
...
function canActorOperateTask(task, candidates, actor, options = {}) {
  if (!actor.userId) return true;
...
async function waitForHumanNode(payload, store, waits, node, nodeInstance, actor) {
  ...
  if (!initialTasks.length) {
    ...
    void emitTaskCreatedNotifications(payload, store, waits, tasks, taskInputs, actor).catch(
...
async function emitTaskCreatedNotifications(payload, store, waits, tasks, taskInputs, actor) {
  await Promise.all(
...
function recipientIdsForTask(task, input) {
  return [
    ...new Set(
      [
        task.assigneeId,
...
async function waitForAnyTaskDecision(tasks, waits) {
  if (!tasks.length) {
...
function waitForTaskDecision(task, waits) {
  if (!task.waitpointTokenId) {
    throw new Error(`Task "${task.id}" has no waitpoint token.`);
  }
  return waits.waitForToken(task.waitpointTokenId);
}
...
function isHumanNodeCompleted(node, tasks) {
...
//# sourceMappingURL=chunk-TEXNLIEG.mjs.map
