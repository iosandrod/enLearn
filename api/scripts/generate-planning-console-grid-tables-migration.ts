...
export const PLANNING_CONSOLE_GRID_TABLES_MIGRATION_FILE =
...
function mappingSql() {
  return Object.entries(PLANNING_CONSOLE_GRID_TABLES)
    .map(([gridId, tableName]) => `(${sqlString(gridId)}, ${sqlString(tableName)})`)
    .join(',\n      ');
}

export function buildPlanningConsoleGridTablesMigrationSql() {
  return `-- Link planning-console aggregate grids to their physical source tables.
...
begin
  ...
  if v_matched_grid_count < ${Object.keys(PLANNING_CONSOLE_GRID_TABLES).length} then
    raise exception 'Expected at least ${Object.keys(PLANNING_CONSOLE_GRID_TABLES).length} planning console grids, found %', v_matched_grid_count;
  end if;
...
export async function generatePlanningConsoleGridTablesMigration() {
  const repoRoot = process.cwd().toLowerCase().endsWith('api')
    ? resolve(process.cwd(), '..')
    : process.cwd();
  const target = resolve(repoRoot, PLANNING_CONSOLE_GRID_TABLES_MIGRATION_FILE);
  const sql = buildPlanningConsoleGridTablesMigrationSql();

  if (process.argv.includes('--check')) {
    const current = await readFile(target, 'utf8');
    if (current !== sql) {
      throw new Error(`Planning console grid-tables migration is stale: ${target}`);
    }
...
if (require.main === module) {
  void generatePlanningConsoleGridTablesMigration();
}
