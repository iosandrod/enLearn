...
async function main() {
  ...
  try {
    ...
    if (counts.get('page')?.material_count !== 17 || counts.get('page')?.source_count !== 17) {
      throw new Error('Expected 17 published Page material sources.');
    }
    ...
    console.log('Low-code material migration applied: 17 Page, 18 Form.');
  } catch (error) {
