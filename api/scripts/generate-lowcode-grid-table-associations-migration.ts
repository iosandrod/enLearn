...
import {
  LOWCODE_GRID_PLANNING_DATASET_TABLES,
  LOWCODE_GRID_TABLE_OVERRIDES,
  LOWCODE_GRID_TABLE_SERVICE_MAPPINGS,
  LOWCODE_GRID_WORKFLOW_ITEM_TABLES
} from './lowcode-grid-table-associations';

export const LOWCODE_GRID_TABLE_ASSOCIATIONS_MIGRATION_FILE =
...
function valueRows(values: readonly (readonly string[])[]) {
  return values.map((row) => `(${row.map(sqlString).join(', ')})`).join(',\n      ');
}

function objectRows(values: Readonly<Record<string, string>>) {
  return valueRows(Object.entries(values));
}

export function buildLowCodeGridTableAssociationsMigrationSql() {
  return `-- Associate every resolvable low-code Grid with its bare public physical-table name.
  ...
  case jsonb_typeof(p_document)
    when 'object' then
      if p_document->>'kind' = 'grid' then
        ...
        from (values
          ${objectRows(LOWCODE_GRID_WORKFLOW_ITEM_TABLES)}
        ...
        from (values
          ${objectRows(LOWCODE_GRID_PLANNING_DATASET_TABLES)}
        ...
        from (values
          ${valueRows(LOWCODE_GRID_TABLE_SERVICE_MAPPINGS)}
        ...
        from (values
          ${objectRows(LOWCODE_GRID_TABLE_OVERRIDES)}
...
export async function generateLowCodeGridTableAssociationsMigration() {
  const repoRoot = process.cwd().toLowerCase().endsWith('api')
    ? resolve(process.cwd(), '..')
    : process.cwd();
  const target = resolve(repoRoot, LOWCODE_GRID_TABLE_ASSOCIATIONS_MIGRATION_FILE);
  const sql = buildLowCodeGridTableAssociationsMigrationSql();

  if (process.argv.includes('--check')) {
    const current = await readFile(target, 'utf8');
    if (current !== sql) {
      throw new Error(`Low-code grid table-association migration is stale: ${target}`);
    }
...
if (require.main === module) {
  void generateLowCodeGridTableAssociationsMigration();
}
