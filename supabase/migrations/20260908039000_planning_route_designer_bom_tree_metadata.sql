-- Ensure the persisted generic grid keeps the VXE tree metadata after
-- visual-designer normalization. The operation column remains hidden/omitted.
begin;
...
with updated as (
  ...
  set schema = jsonb_set(
    ...
    '{blocks,0,blocks,0,blocks,1,schema,grid,columns,0,treeNode}',
    'true'::jsonb,
    true
  ),
...
begin
  if not exists (
    ...
    where page.code = 'planning_route_designer'
      ...
      and page.schema #>> '{blocks,0,blocks,0,blocks,1,schema,grid,columns,0,treeNode}' = 'true'
  ) then
    raise exception 'The route designer BOM tree metadata could not be installed.';
  end if;
end;
$verify$;
