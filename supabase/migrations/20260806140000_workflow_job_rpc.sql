-- Workflow job persistence boundary.
--
-- Trigger.dev remains the external scheduler/executor. All application
-- database mutations and worker reads go through this service-role-only RPC,
-- so API and Trigger.dev code no longer need a direct PostgreSQL connection.
