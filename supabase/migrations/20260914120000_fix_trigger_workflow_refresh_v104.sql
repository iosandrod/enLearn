-- Query workflow jobs when hydrating the Trigger workflow designer.
-- The model list contains wf_model.id values, while runJob expects wf_job.id.
do $migration$
declare
  old_refresh text := $old$async function refresh() {
  const jobs = await host.getServiceApi().invoke<any[]>('workflow', 'listItems', { tableName: 'wf_model' });
...
begin
  ...
  if position('itemType: ''jobs''' in current_source) > 0
     and position('workflowJob.value = jobs.find((item) => item.code === model.value.code);' in current_source) > 0 then
  ...
  if position('invoke<any[]>(''workflow'', ''listItems'', { itemType: ''jobs'' })' in updated_source) = 0
        ...
        select jsonb_agg(
          case
            when block.value->>'kind' = 'trigger-workflow-designer'
              then jsonb_set(block.value, '{materialVersion}', '"1.0.4"'::jsonb, true)
            else block.value
          end
...
where page.code = 'trigger-workflow-designer'
  and exists (
    ...
    where block.value->>'kind' = 'trigger-workflow-designer'
      and block.value->>'materialVersion' is distinct from '1.0.4'
  );
