...
for b in frepple.buffers():
 if b.item.name=='FG-CTRL-100':
  print('before',b.name,b.producing,[(x.operation.name,x.quantity) for x in b.flows])
frepple.settings.suppressFlowplanCreation=False
for b in frepple.buffers():
 if b.item.name=='FG-CTRL-100':
  print('after setting',b.name,b.producing,[(x.operation.name,x.quantity) for x in b.flows])
...
for b in frepple.buffers():
 if b.item.name=='FG-CTRL-100': print('after solve',b.name,b.producing,[(x.operation.name,x.quantity) for x in b.flows])
