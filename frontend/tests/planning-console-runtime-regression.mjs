...
assert.match(flowSource, /const relation = readString\(row\.relation, 'routing'\)/);
assert.match(flowSource, /relation !== 'routing'/);
assert.doesNotMatch(flowSource, /前置约束/);
assert.doesNotMatch(flowSource, /strokeDasharray/);
assert.doesNotMatch(flowSource, /border-style: dashed/);
