const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/router-uVrzr7RE.js","assets/router-B9U0MKye.js","assets/vue-router-BmwJaFrJ.js","assets/vue.runtime.esm-bundler-vRG5b11Z.js","assets/useAuth-B3b6gc1R.js","assets/useAuthState-CuKE8aJm.js"])))=>i.map(i=>d[i]);
import{O as e,lt as t,m as n,rt as r}from"./vue.runtime.esm-bundler-vRG5b11Z.js";var i=`modulepreload`,a=function(e){return`/`+e},o={},s=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),s=document.querySelector(`meta[property=csp-nonce]`),c=s?.nonce||s?.getAttribute(`nonce`);function l(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}function u(e){return import.meta.resolve?import.meta.resolve(e):new URL(e,import.meta.url).href}r=l(t.map(t=>{if(t=a(t,n),t=u(t),t in o)return;o[t]=!0;let r=t.endsWith(`.css`);for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}let s=document.createElement(`link`);if(s.rel=r?`stylesheet`:i,r||(s.as=`script`),s.crossOrigin=``,s.href=t,c&&s.setAttribute(`nonce`,c),document.head.appendChild(s),r)return new Promise((e,n)=>{s.addEventListener(`load`,e),s.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function s(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&s(e.reason);return e().catch(s)})},c=`---\r
...
\r
Happy building with Hikari!`,l=`---\r
...
\r
Start building your next big idea with Hikari today and join the growing community of developers leveraging this powerful SaaS starter!`,ee=`---\r
...
\`\`\`\r
\r
This setup allows users to upload a new avatar image, which is then stored in Supabase Storage and the URL is updated in the user's profile in the database.`,ae=`---\r
...
\r
`,C=Object.assign({"../../content/blog/complete-setup.mdx":c,"../../content/blog/integration-supacrawler.mdx":l,"../../content/blog/introduction.mdx":u,"../../content/blog/roadmap.mdx":ee,"../../content/blog/what-is-hikari.mdx":d,"../../content/blog/why-build-hikari.mdx":te}),oe=Object.assign({"../../content/docs/configure/index.mdx":ne,"../../content/docs/configure/stripe/index.mdx":f,"../../content/docs/configure/stripe/local.mdx":re,"../../content/docs/configure/stripe/production.mdx":ie,"../../content/docs/configure/supabase/index.mdx":p,"../../content/docs/configure/supabase/local.mdx":m,"../../content/docs/configure/supabase/supabase.mdx":h,"../../content/docs/configure/vercel.mdx":g,"../../content/docs/index.mdx":_,"../../content/docs/quick-start.mdx":v,"../../content/docs/storage/example.mdx":y,"../../content/docs/storage/setting-up.mdx":ae,"../../content/docs/test.mdx":b,"../../content/docs/trpc/example.mdx":x,"../../content/docs/trpc/setup.mdx":S});function w(e){return e.replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`).replaceAll(`"`,`&quot;`).replaceAll(`'`,`&#039;`)}function T(e){return w(e).replaceAll("`",`&#096;`)}function se(e){let t=e.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n?([\s\S]*)$/);return t?{data:t[1].split(/\r?\n/).reduce((e,t)=>{let n=t.indexOf(`:`);if(n===-1)return e;let r=t.slice(0,n).trim();return e[r]=t.slice(n+1).trim().replace(/^["']|["']$/g,``),e},{}),body:t[2].trim()}:{data:{},body:e.trim()}}function ce(e){return e.toLowerCase().trim().replace(/[^a-z0-9]+/g,`-`).replace(/^-+|-+$/g,``)}function le(e,t){let n=ce(e)||`section`,r=t.get(n)??0;return t.set(n,r+1),r===0?n:`${n}-${r+1}`}function E(e){let t=[],n=w(e);return n=n.replace(/`([^`]+)`/g,(e,n)=>{let r=`__INLINE_CODE_${t.length}__`;return t.push(`<code>${n}</code>`),r}),n=n.replace(/\[([^\]]+)\]\(([^)]+)\)/g,(e,t,n)=>`<a href="${T(n)}">${t}</a>`),n=n.replace(/\*\*([^*]+)\*\*/g,`<strong>$1</strong>`).replace(/\*([^*]+)\*/g,`<em>$1</em>`),t.forEach((e,t)=>{n=n.replace(`__INLINE_CODE_${t}__`,e)}),n}function D(e){let t={};for(let n of e.matchAll(/(\w+)=["']([^"']*)["']/g))t[n[1]]=n[2];return t}function O(e){let t=[...e.matchAll(/<Card\s+([^>]+?)\/>/g)].map(e=>D(e[1]));return t.length?[`<div class="docs-card-grid">`,...t.map(e=>{let t=w(e.title??`Open page`),n=e.description?`<span class="docs-card-description">${w(e.description)}</span>`:``;return`<a class="docs-card" href="${T(e.href??`#`)}"><span class="docs-card-title">${t}</span>${n}</a>`}),`</div>`].join(``):``}function k(e){let t=new Map,n=e.replace(/<Cards>([\s\S]*?)<\/Cards>/g,(e,n)=>{let r=`__DOC_CARD_BLOCK_${t.size}__`;return t.set(r,O(n)),`\n${r}\n`}),r=[],i=[],a=new Map;for(let e of n.split(/\n{2,}/)){let n=e.trim();if(!n)continue;if(t.has(n)){r.push(n);continue}if(n.startsWith("```")){let e=n.replace(/^```[^\n]*\n?/,``).replace(/\n?```$/,``);r.push(`<pre><code>${w(e)}</code></pre>`);continue}let o=n.match(/^(#{1,3})\s+(.+)$/);if(o){let e=o[1].length,t=o[2].trim(),n=le(t,a);i.push({depth:e,id:n,text:t}),r.push(`<h${e} id="${n}">${E(t)}</h${e}>`);continue}if(/^[-*]\s+/m.test(n)){r.push(`<ul>${n.split(/\r?\n/).map(e=>`<li>${E(e.replace(/^[-*]\s+/,``))}</li>`).join(``)}</ul>`);continue}r.push(`<p>${E(n.replace(/\r?\n/g,` `))}</p>`)}let o=r.join(`
`);for(let[e,n]of t.entries())o=o.replaceAll(e,n);return{html:o,toc:i}}function A(e,t){let[,n=``]=e.replace(/\\/g,`/`).split(`/content/${t}/`),r=n.replace(/\.mdx$/,``).split(`/`);return r.at(-1)===`index`&&r.pop(),r.join(`/`)}function j(e){let t=e.replace(/```[\s\S]*?```/g,``);for(let e of t.split(/\r?\n/)){let t=e.trim();if(!(!t||t.startsWith(`#`)||t.startsWith(`<`)||t.startsWith(`- `)||/^\d+\.\s/.test(t)))return t.replace(/\[([^\]]+)\]\(([^)]+)\)/g,`$1`).replace(/[*`]/g,``).slice(0,180)}return``}function M(e,t,n,r){let{data:i,body:a}=se(t),o=A(e,n),s=k(a),c=i.title??o.split(`/`).at(-1)??`Untitled`,l=i.description??j(a);return{slug:o,href:o?`${r}/${o}`:r,title:c,description:l,date:i.date,author:i.author,excerpt:l,bodyHtml:s.html,toc:s.toc}}function N(e){let t=Object.entries(C).map(([e,t])=>{let{bodyHtml:n,toc:r,...i}=M(e,t,`blog`,`/blog`);return i}).sort((e,t)=>new Date(t.date??t.slug).getTime()-new Date(e.date??e.slug).getTime());return typeof e==`number`?t.slice(0,e):t}function P(e){let t=Object.entries(C).find(([t])=>A(t,`blog`)===e);if(!t)throw Error(`Blog post not found`);return M(t[0],t[1],`blog`,`/blog`)}function F(e){let t=Object.entries(oe).find(([t])=>A(t,`docs`)===e);if(!t)throw Error(`Doc page not found`);return M(t[0],t[1],`docs`,`/docs`)}async function I(e){let t=new URL(e,window.location.origin);if(t.pathname===`/api/content/blog`){let e=Number(t.searchParams.get(`limit`));return N(Number.isFinite(e)&&e>0?e:void 0)}if(t.pathname.startsWith(`/api/content/blog/`))return P(decodeURIComponent(t.pathname.replace(`/api/content/blog/`,``)));if(t.pathname===`/api/content/docs`)return F(``);if(t.pathname.startsWith(`/api/content/docs/`))return F(decodeURIComponent(t.pathname.replace(`/api/content/docs/`,``)));throw Error(`Unsupported content URL: ${e}`)}e(),r();var L=new Map,R=`enlearn_access_token`,z=`enlearn_refresh_token`,B=null;function V(){return`http://localhost:3002/api`.replace(/\/+$/,``)}function H(e){return e.replace(/^\/api/,``)}function U(){return localStorage.getItem(R)||``}function W(){return localStorage.getItem(z)||``}function G(e){let t=e?.session;t?.access_token&&(localStorage.setItem(R,t.access_token),t.refresh_token&&localStorage.setItem(z,t.refresh_token))}function K(){localStorage.removeItem(R),localStorage.removeItem(z)}function q(e,t){if(!t)return e;let n=new URL(e,window.location.origin);for(let[e,r]of Object.entries(t))r!=null&&n.searchParams.set(e,String(r));return n.pathname+n.search+n.hash}function J(e){if(!e)return null;try{return JSON.parse(e)}catch{return{message:e}}}function ue(e,t){if(typeof e==`string`)return e;if(!e||typeof e!=`object`)return t;let n=e,r=Array.isArray(n.message)?n.message.filter(Boolean).join(`, `):n.message;return String(r??n.statusMessage??n.error??t)}function Y(e,t){let n=t.toLowerCase();return e===401||n.includes(`jwt expired`)||n.includes(`invalid jwt`)||n.includes(`invalid token`)||n.includes(`authentication required`)}function de(e){return!e.startsWith(`/auth/refresh`)&&!e.startsWith(`/auth/signin`)&&!e.startsWith(`/auth/signup`)}function fe(e,t,n){if(t)return t;if(n!==void 0&&(e.startsWith(`/auth/signin`)||e.startsWith(`/auth/signup`)||e.startsWith(`/auth/session`)||e.startsWith(`/auth/signout`)||e.startsWith(`/auth/refresh`)||e.startsWith(`/service`)))return`POST`}function X(){window.location.pathname!==`/signin`&&$(`/signin`).catch(()=>{window.location.href=`/signin`})}function pe(){let e=!!(U()||W());K(),(e||window.location.pathname.startsWith(`/dashboard`))&&X()}async function me(){let e=W();return e?(B||=(async()=>{let t=await fetch(`${V()}/auth/refresh`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({refreshToken:e})}),n=J(await t.text());return t.ok?(G(n),!!n?.session?.access_token):!1})().catch(()=>!1).finally(()=>{B=null}),B):!1}async function Z(e,t={},n=!1){let r=H(q(e,t.query)),{query:i,body:a,...o}=t,s=new Headers(t.headers),c=U();c&&!s.has(`Authorization`)&&s.set(`Authorization`,`Bearer ${c}`),a!==void 0&&!(a instanceof FormData)&&s.set(`Content-Type`,s.get(`Content-Type`)||`application/json`);let l=await fetch(`${V()}${r}`,{...o,method:fe(r,o.method,a),headers:s,body:a===void 0||a instanceof FormData?a:JSON.stringify(a)}),u=J(await l.text());if(!l.ok){let i=ue(u,l.statusText);if(!n&&Y(l.status,i)&&de(r)&&await me())return Z(e,t,!0);throw Y(l.status,i)&&pe(),Q({statusCode:l.status,statusMessage:i})}return(r.startsWith(`/auth/signin`)||r.startsWith(`/auth/signup`)||r.startsWith(`/auth/session`))&&G(u),r.startsWith(`/auth/signout`)&&K(),u}async function he(e,t={}){let n=H(e),r=String(t.method??`GET`).toUpperCase(),i=n.match(/^\/posts\/([^/]+)$/)?.[1];return r===`GET`&&n===`/posts`?Z(`/api/service`,{method:`POST`,body:{serviceName:`posts`,serviceMethod:`listItems`,postData:{tableName:`posts`}}}):r===`POST`&&n===`/posts`?Z(`/api/service`,{method:`POST`,body:{serviceName:`posts`,serviceMethod:`createItem`,postData:{resource:`posts`,...t.body??{}}}}):r===`PUT`&&i?Z(`/api/service`,{method:`POST`,body:{serviceName:`posts`,serviceMethod:`updateItem`,postData:{resource:`posts`,id:i,...t.body}}}):r===`DELETE`&&i?Z(`/api/service`,{method:`POST`,body:{serviceName:`posts`,serviceMethod:`deleteItem`,postData:{resource:`posts`,id:i}}}):Z(e,t)}function ge(e,n){return L.has(e)||L.set(e,t(n())),L.get(e)}function Q(e){let t=Error(e.statusMessage??e.message??`Application error`);return t.statusCode=e.statusCode,t.statusMessage=e.statusMessage,t}async function $(e){let{router:t}=await s(async()=>{let{router:e}=await import(`./router-uVrzr7RE.js`);return{router:e}},__vite__mapDeps([0,1,2,3,4,5]));return t.push(e)}function _e(e){let t=e.title,r=e.description,i=typeof t==`function`?n(t).value:t,a=typeof r==`function`?n(r).value:r;if(i&&(document.title=String(i)),a){let e=document.querySelector(`meta[name="description"]`);e||(e=document.createElement(`meta`),e.name=`description`,document.head.appendChild(e)),e.content=String(a)}}async function ve(e,n,r={}){let i=t(r.default?r.default():null),a=t(null);try{i.value=await n()}catch(e){a.value=e}return{data:i,error:a}}async function ye(e,t={}){let n=q(e,t.query);return n.startsWith(`/api/content/`)?I(n):n===`/api/auth/socket-token`?{token:U(),socketBaseUrl:`http://localhost:3002`}:n.startsWith(`/api/posts`)?he(n,t):Z(n,t)}export{_e as a,ve as i,Q as n,ge as o,$ as r,s,ye as t};