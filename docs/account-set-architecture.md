...
系统采用类似用友 ERP 的“身份账号 + 业务账套”登录模型。登录页把账号、密码和账套放在同一张表单中，提交后服务端依次完成身份认证与账套权限校验。账套不是浏览器传来的普通筛选条件，而是服务端验证后的安全边界。

## 核心模型

- 系统只支持 ERP 业务账套，不创建、不展示个人账户；历史 `personal_account = true` 记录由下线迁移删除。
...
1. `GET /auth/account-options?login=<登录账号>` 返回该账号可选择的启用账套，不包含成员角色或用户权限。
2. 用户在登录页填写账号、密码并选择账套。
3. `POST /auth/signin` 验证身份后实时校验该用户是否属于所选账套以及账套是否启用，校验成功才建立当前账套上下文。
4. 服务端记录最近使用账套；登录页勾选“下次优先使用”时才更新默认账套。最近账套用于审计和当前会话恢复，不会覆盖用户显式设置的默认账套。
5. 前端保存 `enlearn_active_account_id`，后续请求自动携带 `X-Account-Id`。
6. `GET /auth/me` 恢复会话。若本地账套已停用、归档或成员关系已撤销，接口保留登录身份并返回 `accountRequired: true`，用户可在登录页重新选择其他账套。

开发环境自动登录仍在内部路径自动选择账套；人工登录通过单张表单完成认证和选择。
