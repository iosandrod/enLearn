  ...
  >
    <div class="login-brand">
      ...
      <div class="brand-copy">
        <span class="brand-product">ENLEARN COURSE HUB</span>
        <span class="brand-title">课程培训学习平台</span>
        <span class="brand-description">连接课程、学习进度、作业与教学管理</span>
      </div>
    </div>
    ...
    <div class="login-workspace">
      <div class="login-panel">
        ...
        <span class="login-title">登录课程培训平台</span>
        <span class="login-description">请选择使用身份，再输入账号和密码。</span>
      ...
      <span class="login-footer">EnLearn Course Hub · 移动课程培训平台</span>
    </div>
  </div>
</template>
...
import { MobileForm } from '@enlearn/mobile-lowcode/controls';
import type {
  MobileFormModels,
    ...
    fields: [
      {
        ...
        rules: [{ required: true, message: '请选择使用身份' }],
      },
      {
        field: 'email',
        label: '登录账号',
...
async function submitLogin(values: Record<string, unknown>) {
  ...
  try {
    ...
    await Promise.all([
      ...
      writeMobileStorage('appRole', String(values.role ?? 'student')),
    ]);
...
onMounted(async () => {
  ...
  const savedRole = await readMobileStorage('appRole');
  loginModel.value.role = savedRole === 'teacher' ? 'teacher' : 'student';
