...
assert.deepEqual(
  requests.normalizeMobileServiceRequest('admin', 'listWorkflowJobRuns', { limit: 20 }),
  {
    ...
    serviceMethod: 'listItems',
    postData: { limit: 20, pageSize: 20, tableName: 'wf_job_run' },
  },
);
assert.deepEqual(
  requests.normalizeMobileServiceRequest('admin', 'listWorkflowJobs', {
    itemType: 'jobs',
    resource: 'jobs',
    type: 'jobs',
  }),
  {
    serviceName: 'workflow',
    serviceMethod: 'listItems',
    postData: { tableName: 'wf_job' },
  },
  'workflow compatibility requests must not forward legacy list item selectors',
);
assert.deepEqual(
  requests.normalizeMobileServiceRequest('admin', 'listWorkflowJobRuns', { limit: 500 }),
  {
    serviceName: 'workflow',
    serviceMethod: 'listItems',
    postData: { limit: 100, pageSize: 100, tableName: 'wf_job_run' },
  },
...
assert.deepEqual(
  requests.normalizeMobileServiceRequest('admin', 'listWorkflowTimerJobs', { limit: 20 }),
  {
    serviceName: 'workflow',
    serviceMethod: 'listItems',
    postData: { limit: 20, filters: { type: 'cron' }, tableName: 'wf_job' },
  },
);
assert.deepEqual(
