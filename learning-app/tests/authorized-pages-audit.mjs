...
function normalizeDataSourceRequest(source) {
  ...
  const workflowTables = {
    listSystemExecutionTasks: 'wf_job', listWorkflowJobs: 'wf_job',
    listWorkflowJobRuns: 'wf_job_run', listWorkflowTimerJobs: 'wf_job',
  };
  ...
  if (serviceName === 'admin' && workflowTables[serviceMethod]) {
    const tableName = workflowTables[serviceMethod];
    const requestedLimit = Number(postData.limit ?? postData.pageSize ?? postData.page_size);
    serviceName = 'workflow';
    serviceMethod = 'listItems';
    postData.tableName = tableName;
    if (originalServiceMethod === 'listWorkflowJobRuns') {
