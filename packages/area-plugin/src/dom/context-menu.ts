...
interface ContextMenuRefs {
  menuEl: HTMLDivElement
  insertRowsButton: HTMLButtonElement
  insertColumnsButton: HTMLButtonElement
  removeRowsButton: HTMLButtonElement
  removeColumnsButton: HTMLButtonElement
...
const tableContextMenuRefsMap = new WeakMap<any, ContextMenuRefs>()
...
function getCellFromTarget ($table: any, target: EventTarget | null) {
...
function getBodyCellFromEvent ($table: any, evnt: MouseEvent) {
...
function isCellInActiveArea ($table: any, cell: HTMLElement) {
...
function createMenuButton (code: string, label: string) {
...
function createContextMenuRefs ($table: any): ContextMenuRefs {
  ...
  const handleMenuMousedown = (evnt: MouseEvent) => {
    evnt.preventDefault()
    evnt.stopPropagation()
  }
  const runAction = (button: HTMLButtonElement, evnt: MouseEvent, callback: () => void) => {
    evnt.preventDefault()
    evnt.stopPropagation()
    if (!button.disabled) {
...
function getContextMenuRefs ($table: any) {
...
function bindDocumentCloseListeners ($table: any, refs: ContextMenuRefs) {
  if (refs.removeDocumentListeners) {
    return
  }

  const hide = () => hideCellAreaContextMenu($table)
  const handleDocumentMousedown = (evnt: MouseEvent) => {
    if (isEventTargetNode(evnt.target) && refs.menuEl.contains(evnt.target)) {
      return
    }
    hide()
  }
  const handleDocumentContextmenu = (evnt: MouseEvent) => {
    if (isEventTargetNode(evnt.target) && refs.menuEl.contains(evnt.target)) {
      evnt.preventDefault()
      return
    }
...
function positionMenu (refs: ContextMenuRefs, clientX: number, clientY: number) {
  const { menuEl } = refs
...
export function hideCellAreaContextMenu ($table: any) {
...
export function unbindCellAreaContextMenu ($table: any) {
...
export function bindCellAreaContextMenu ($table: any) {
...
export function registerCellAreaContextMenuLifecycle (VxeUI: VxeUILike) {
