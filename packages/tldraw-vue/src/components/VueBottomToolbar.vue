...
import { TopMenuController } from '@/editor/interactions/TopMenuController'
...
const topMenuController = new TopMenuController(props.editor)
...
const primaryItems = computed(() =>
	new ToolbarController(props.toolbarTools).getPrimaryItems(props.activeTool, props.currentGeoShape)
)
const canUndo = useEditorValue('bottom toolbar can undo', () => topMenuController.getCanUndo())
const canRedo = useEditorValue('bottom toolbar can redo', () => topMenuController.getCanRedo())
const hasSelection = useEditorValue('bottom toolbar has selection', () =>
	topMenuController.getHasSelection()
)
...
const actionItems = computed(() => [
	...
	},
	{
		...
		run: () => topMenuController.deleteSelection(),
	},
	{
		id: 'duplicate',
		label: 'Duplicate',
		glyph: glyph(0x29c9),
		disabled: !hasSelection.value,
		run: () => topMenuController.duplicateSelection(),
	},
])

function closeMenus() {
	// Kept for the parent component API; this toolbar no longer owns a popover.
}
...
function runAction(action: (typeof actionItems.value)[number]) {
...
<template>
	...
	>
		<div class="bottom-toolbar">
			...
			<div class="bottom-toolbar-separator" />

			<button
				v-for="action in actionItems"
				:key="action.id"
				type="button"
				class="bottom-toolbar-button bottom-toolbar-button--action"
