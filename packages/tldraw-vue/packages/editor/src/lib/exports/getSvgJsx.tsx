...
import { svgExportNodeToReact } from './svgExportNodeToReact'
...
function SvgExport({
	...
	const addExportDef = useEvent((def: SvgExportDef) => {
		stateAtom.update((state) => {
			...
			const promise = Promise.resolve(def.getElement()).then((result) => svgExportNodeToReact(result))
			waitUntil(
	...
	useLayoutEffect(() => {
		...
		;(async () => {
			const shapeDefs: Record<string, { pending: false; element: ReactNode }> = {}
				...
				async ({ id, opacity, index, backgroundIndex }) => {
					...
					if (util.toSvg || util.toBackgroundSvg) {
						...
						if (toBackgroundSvgResult) {
							const toBackgroundSvgElement = svgExportNodeToReact(toBackgroundSvgResult)
							elements.push({
