  ...
  return normalizeTriggerWorkflow({
    ...
    nodes: [
      ...
      node('task', 'validate_expense', '校验报销数据', 380, 190, {
        task: { type: 'backendCommand', commandCode: 'expense.validate', retry: { maxAttempts: 3 }, idempotencyKey: '{{payload.expenseId}}:validate' }
      }),
      node('condition', 'amount_condition', '判断报销金额', 380, 340),
      node('manualApproval', 'manager_approval', '经理审批', 190, 510, {
        task: { type: 'backendCommand', commandCode: 'approval.manager.wait' },
      ...
      node('manualApproval', 'finance_approval', '财务审批', 570, 510, {
        task: { type: 'backendCommand', commandCode: 'approval.finance.wait' },
      ...
      node('task', 'sync_status', '同步审批结果', 380, 690, {
        task: { type: 'backendCommand', commandCode: 'expense.syncApprovalStatus', retry: { maxAttempts: 5 } }
      }),
  ...
  return normalizeTriggerWorkflow({
    ...
    nodes: [
      ...
      node('memory', 'load_context', '加载客户上下文', 380, 190, {
        task: { type: 'backendCommand', commandCode: 'agent.memory.load' },
        ai: { memoryKey: '{{payload.customerId}}' }
      }),
      node('agent', 'triage_agent', '智能分诊', 380, 340, {
        task: { type: 'backendCommand', commandCode: 'agent.support.triage' },
        ai: {
          provider: 'openai',
          model: 'gpt-4.1',
      ...
      node('tool', 'search_docs', '检索知识库', 180, 650, {
        task: { type: 'backendCommand', commandCode: 'agent.tool.searchDocs' },
        ai: { tools: ['search_docs'] }
      }),
      node('tool', 'draft_reply', '生成回复草稿', 580, 650, {
        task: { type: 'backendCommand', commandCode: 'agent.tool.draftReply' },
        ai: { tools: ['create_reply'] }
      }),
      node('humanReview', 'review_reply', '人工复核回复', 380, 830, {
        task: { type: 'backendCommand', commandCode: 'agent.review.wait' },
      ...
      node('task', 'send_reply', '发送回复', 380, 990, {
        task: { type: 'backendCommand', commandCode: 'support.sendReply', idempotencyKey: '{{payload.ticketId}}:reply' }
      }),
