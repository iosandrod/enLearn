...
export function resetScales(
	...
	const rows = scales.map(line => {
		...
		return {
			cells,
			add,
			height: fullHeight / scales.length,
		};
	});
...
export function expandScale(
	...
	if (start && end) {
		if (width < minWidth && width) {
			...
			return {
				cellWidth: Math.ceil(cellWidth * k),
			};
		}
		return {};
	}
...
export function calcScaleDate(x: number, state: Partial<IData>) {
	const { _scales, _start, _weekStart } = state;
	const width =
