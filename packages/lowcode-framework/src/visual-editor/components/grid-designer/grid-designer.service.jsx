import { computed, createApp, defineComponent, getCurrentInstance, nextTick, onMounted, reactive, } from 'vue';
import DesignerUI, { ElButton, ElDialog, ElForm, ElFormItem, ElInput, ElMessage, ElOption, ElSelect, ElSwitch, ElTabPane, ElTable, ElTableColumn, ElTabs, } from '../common/designer-ui';
...
function assertJsonParsed(result) {
    if (result.ok === false) {
...
const advancedGridConfigDefinitions = [
...
const columnAdvancedFormSections = [
    {
        title: '尺寸与对齐',
        fields: [
            {
...
function parseJsonObject(value, label) {
    ...
    if (!parsed.ok || typeof parsed.value === 'undefined')
        return parsed;
...
function readJsonArrayValue(value) {
    if (Array.isArray(value))
...
function parseJsonValueOrString(value, label) {
...
function createDefaultColumn(index = 0) {
    return {
...
function normalizeColumn(column, index) {
    ...
    return {
        ...
        showOverflow: row.showOverflow === undefined ? '' : row.showOverflow,
        showHeaderOverflow: row.showHeaderOverflow === undefined ? '' : row.showHeaderOverflow,
        showFooterOverflow: row.showFooterOverflow === undefined ? '' : row.showFooterOverflow,
...
function normalizeGridOptions(value) {
    ...
    const extraProps = Object.fromEntries(Object.entries(gridOptions).filter(([key]) => !commonGridOptionKeys.has(key)));
    return {
        options: {
            ...defaults,
            ...gridOptions,
...
function readAdvancedConfigObject(advanced, field, label) {
...
function createAdvancedFormModels(advanced) {
    return {
        ...advancedGridConfigDefinitions.reduce((models, config) => {
...
function normalizeEvents(value) {
    ...
    return eventDefinitions.map((definition) => {
        ...
        const directives = normalizeRuntimeDirectives(Array.isArray(row.directives)
            ? row.directives
...
function resetReactiveObject(target, source) {
    Object.keys(target).forEach((key) => delete target[key]);
    Object.assign(target, source);
}
function validateColumnJson(_column, _index) {
    return { ok: true, value: undefined };
}
function parseFormatterValue(value, label) {
...
function inferObjectFields(value) {
...
function createObjectSchema(value, fields) {
    return {
        fields: fields?.length ? fields : inferObjectFields(value),
        actions: [],
    };
}
const directiveArrayColumns = [
...
function inferArrayColumns(value) {
    const keys = new Set();
...
function createArrayDefaultRow(columns) {
...
function createArraySchema(value, columns) {
...
function normalizeColumnForResult(column, index) {
...
function normalizeRuntimeDirectives(value) {
    return Array.isArray(value)
        ? value.filter((item) => isPlainRecord(item) && typeof item.type === 'string' && item.type.trim().length > 0)
        : [];
}
function buildGridOptions(options, advanced) {
...
const ServiceComponent = defineComponent({
    ...
    setup(props) {
        ...
        const selectedColumn = computed(() => state.columns.find((column) => column.__id === state.selectedColumnId) ??
            state.columns[0] ??
            null);
        const selectedColumnIndex = computed(() => selectedColumn.value
            ? state.columns.findIndex((column) => column.__id === selectedColumn.value?.__id)
            : -1);
        const selectColumn = (column) => {
            state.selectedColumnId = readString(column.__id);
        };
        const syncSelectedColumn = (value) => {
        ...
        const handler = {
            onConfirm: () => {
                try {
                    ...
                    const invalidColumn = state.columns.find((column) => !readString(column.field) && !readString(column.title) && !readString(column.type));
                    if (invalidColumn) {
        ...
        const openObjectEditor = (title, value, fields, onConfirm) => {
        ...
        const openArrayEditor = (title, value, columns, onConfirm) => {
        ...
        const renderObjectBoundInput = (row, field, title, placeholder, fields) => {
            const value = row[field];
            if (!isPlainRecord(value)) {
                return (<ElInput modelValue={readString(value)} placeholder={placeholder} {...{
                    'onUpdate:modelValue': (nextValue) => {
                        row[field] = nextValue;
                    },
                }}/>);
            }
            return (<div class="grid-designer-object-cell">
          <ElInput modelValue={formatObjectSummary(value)} placeholder={placeholder} readonly/>
          <ElButton text type="primary" onClick={(event) => {
        ...
        const renderArrayEditorInput = (row, field, title, columns) => (<div class="grid-designer-object-cell">
        ...
        const syncAdvancedConfigModel = (field, value) => {
            const nextValue = isPlainRecord(value) ? cloneDeep(value) : {};
            state.advancedModels[field] = nextValue;
            state.advanced[field] = JSON.stringify(compactObject(cloneDeep(nextValue)), null, 2);
        };
        const renderAdvancedConfigEditor = (config) => {
            const model = state.advancedModels[config.field] ?? {};
            return (<section key={config.field} class="grid-designer-advanced-item">
          <div class="grid-designer-advanced-item__header">
            <strong>{config.label}</strong>
          </div>
          <LowCodeForm class="grid-designer-advanced-sub-form" schema={{ fields: config.fields, actions: [] }} modelValue={model} onUpdate:modelValue={(value) => {
                    syncAdvancedConfigModel(config.field, value);
                }}/>
        </section>);
        };
        const renderExtraPropsEditor = () => {
            const model = state.advancedModels.extraPropsJson ?? {};
            return (<section class="grid-designer-advanced-item grid-designer-advanced-item--wide">
          <div class="grid-designer-advanced-item__header">
            <strong>extraProps</strong>
          </div>
          <div class="grid-designer-object-cell">
            <ElInput modelValue={formatObjectSummary(model)} readonly/>
            <ElButton text type="primary" onClick={() => {
        ...
        const renderBusinessPostDataEditor = () => (<div class="grid-designer-object-cell">
        ...
        const renderColumnToolbar = () => (<div class="grid-designer-column-toolbar">
        ...
        const renderColumnRowActions = (index) => (<div class="grid-designer-row-actions">
        <span title="上移">
          <ElButton circle text type="primary" icon={ArrowUp} disabled={index <= 0} onClick={() => columnActions.move(index, -1)}/>
        </span>
        <span title="下移">
          <ElButton circle text type="primary" icon={ArrowDown} disabled={index >= state.columns.length - 1} onClick={() => columnActions.move(index, 1)}/>
        </span>
        <span title="复制">
          <ElButton circle text type="primary" icon={CopyDocument} onClick={() => columnActions.copy(index)}/>
        </span>
        <span title="删除">
          <ElButton circle text type="danger" icon={Delete} disabled={state.columns.length <= 1} onClick={() => columnActions.remove(index)}/>
        </span>
      </div>);
        const renderColumnDesigner = () => (<div class="grid-designer-panel">
        {renderColumnToolbar()}
        <ElTable data={state.columns} border height="460" rowKey="__id" class="grid-designer-table">
          ...
          <ElTableColumn label="field" minWidth={150}>
            {{
                default: ({ row }) => (<ElInput v-model={row.field} placeholder="field"/>),
            }}
          </ElTableColumn>
          <ElTableColumn label="title" minWidth={150}>
            {{
                default: ({ row }) => (<ElInput v-model={row.title} placeholder="title"/>),
            }}
          </ElTableColumn>
          <ElTableColumn label="type" width={132}>
            {{
                default: ({ row }) => (<ElSelect v-model={row.type}>{renderSelectOptions(columnTypeOptions)}</ElSelect>),
            }}
          </ElTableColumn>
          <ElTableColumn label="width" width={108}>
            {{
                default: ({ row }) => (<ElInput v-model={row.width} placeholder="auto"/>),
            }}
          </ElTableColumn>
          <ElTableColumn label="minWidth" width={118}>
            {{
                default: ({ row }) => (<ElInput v-model={row.minWidth} placeholder="minWidth"/>),
            }}
          </ElTableColumn>
          <ElTableColumn label="fixed" width={116}>
            {{
                default: ({ row }) => (<ElSelect v-model={row.fixed}>{renderSelectOptions(fixedOptions)}</ElSelect>),
            }}
          </ElTableColumn>
          <ElTableColumn label="align" width={124}>
            {{
                default: ({ row }) => (<ElSelect v-model={row.align}>{renderSelectOptions(alignOptions)}</ElSelect>),
            }}
          </ElTableColumn>
          <ElTableColumn label="sortable" width={96} align="center">
            {{
                default: ({ row }) => <ElSwitch v-model={row.sortable}/>,
            }}
          </ElTableColumn>
          <ElTableColumn label="visible" width={92} align="center">
            {{
                default: ({ row }) => <ElSwitch v-model={row.visible}/>,
            }}
          </ElTableColumn>
          <ElTableColumn label="showOverflow" width={138}>
            {{
                default: ({ row }) => (<ElSelect v-model={row.showOverflow}>{renderSelectOptions(overflowOptions)}</ElSelect>),
            }}
          </ElTableColumn>
          <ElTableColumn label="formatter" minWidth={210}>
            {{
                default: ({ row }) => (renderObjectBoundInput(row, 'formatter', `${readString(row.title, readString(row.field, '列'))} formatter`, '{"type":"text"} 或格式化器名', formatterObjectFields)),
            }}
          </ElTableColumn>
          <ElTableColumn label="操作" width={144} align="center">
            {{
                default: ({ $index }) => renderColumnRowActions($index),
            }}
          </ElTableColumn>
        </ElTable>
      </div>);
        const renderColumnDesignerWorkbench = () => {
            ...
            const fallbackTitle = selectedColumnIndex.value >= 0 ? `Column ${selectedColumnIndex.value + 1}` : 'Column';
            return (<div class="grid-designer-panel">
          <div class="grid-designer-column-layout">
            <section class="grid-designer-column-table-panel">
              {renderColumnToolbar()}
              <ElTable data={state.columns} border height="100%" rowKey="__id" class="grid-designer-table" {...{
                rowClassName: ({ row }) => row.__id === state.selectedColumnId ? 'is-selected' : '',
                onCellClick: ({ row }) => selectColumn(row),
            }}>
                <ElTableColumn type="index" width={48} label="#"/>
                <ElTableColumn label="field" minWidth={150}>
                  {{
                ...
                <ElTableColumn label="showOverflow" width={138}>
                  {{
                    default: ({ row }) => (<ElSelect v-model={row.showOverflow}>
            ...
            <aside class="grid-designer-column-form-panel">
              <div class="grid-designer-card is-compact grid-designer-column-form-card">
                <div class="grid-designer-column-form-header">
                  ...
                  {selectedColumnIndex.value >= 0 ? (<b>{selectedColumnIndex.value + 1}</b>) : null}
                </div>
                {column ? (<div class="grid-designer-column-form-sections">
                    {columnAdvancedFormSections.map((section) => (<section key={section.title} class="grid-designer-column-form-section">
                        <div class="grid-designer-column-form-section__title">{section.title}</div>
                        <LowCodeForm key={`${column.__id}-${section.title}`} class="grid-designer-column-sub-form" schema={{ fields: section.fields, actions: [] }} modelValue={column} onUpdate:modelValue={syncSelectedColumn}/>
                      </section>))}
                  </div>) : (<div class="grid-designer-empty">请选择左侧表格中的列</div>)}
              </div>
            </aside>
          </div>
        </div>);
        };
        const renderBaseInfo = () => (<ElForm labelPosition="top" class="grid-designer-panel">
        ...
        <div class="grid-designer-card">
          ...
          <div class="grid-designer-form-grid">
            <ElFormItem label="rowConfig.keyField">
              <ElInput v-model={state.gridOptions.rowConfig.keyField}/>
            </ElFormItem>
            <ElFormItem label="rowConfig.useKey">
              <ElSwitch v-model={state.gridOptions.rowConfig.useKey}/>
            </ElFormItem>
            <ElFormItem label="rowConfig.isCurrent">
              <ElSwitch v-model={state.gridOptions.rowConfig.isCurrent}/>
            </ElFormItem>
            <ElFormItem label="rowConfig.isHover">
              <ElSwitch v-model={state.gridOptions.rowConfig.isHover}/>
            </ElFormItem>
            <ElFormItem label="rowConfig.resizable">
              <ElSwitch v-model={state.gridOptions.rowConfig.resizable}/>
            </ElFormItem>
            <ElFormItem label="rowConfig.drag">
              <ElSwitch v-model={state.gridOptions.rowConfig.drag}/>
            </ElFormItem>
            <ElFormItem label="columnConfig.useKey">
              <ElSwitch v-model={state.gridOptions.columnConfig.useKey}/>
            </ElFormItem>
            <ElFormItem label="columnConfig.resizable">
              <ElSwitch v-model={state.gridOptions.columnConfig.resizable}/>
            </ElFormItem>
            <ElFormItem label="columnConfig.isCurrent">
              <ElSwitch v-model={state.gridOptions.columnConfig.isCurrent}/>
            </ElFormItem>
            <ElFormItem label="columnConfig.isHover">
              <ElSwitch v-model={state.gridOptions.columnConfig.isHover}/>
            </ElFormItem>
            <ElFormItem label="columnConfig.drag">
              <ElSwitch v-model={state.gridOptions.columnConfig.drag}/>
            </ElFormItem>
            <ElFormItem label="columnConfig.minWidth">
              <ElInput v-model={state.gridOptions.columnConfig.minWidth}/>
            </ElFormItem>
          </div>
        </div>
        ...
        const renderEventDesigner = () => (<div class="grid-designer-panel">
        <ElTable data={state.gridEvents} border height="560" rowKey="key" class="grid-designer-table">
          <ElTableColumn label="启用" width={78} align="center">
            {{
                default: ({ row }) => <ElSwitch v-model={row.enabled}/>,
            }}
          </ElTableColumn>
          ...
          <ElTableColumn label="运行事件名 eventName" minWidth={220}>
            {{
                default: ({ row }) => (<ElInput v-model={row.eventName} placeholder={`grid.${row.key}`}/>),
            }}
          </ElTableColumn>
          <ElTableColumn label="指令" minWidth={220}>
            {{
                default: ({ row }) => (renderArrayEditorInput(row, 'directives', `${row.vxeName} 指令`, directiveArrayColumns)),
            }}
          </ElTableColumn>
        </ElTable>
      </div>);
        return () => (<>
        <ElDialog v-model={state.showFlag} title={state.option.title || '表格设计'} width="min(1360px, calc(100vw - 40px))" top="4vh" class="grid-designer-dialog form-workbench-dialog" destroyOnClose={true}>
          {{
        ...
        <ElDialog v-model={objectEditor.showFlag} title={objectEditor.title} width="min(720px, calc(100vw - 48px))" top="8vh" class="grid-designer-object-dialog" destroyOnClose={true}>
          {{
                default: () => (<LowCodeForm schema={objectEditor.schema} modelValue={objectEditor.model} onUpdate:modelValue={(value) => {
