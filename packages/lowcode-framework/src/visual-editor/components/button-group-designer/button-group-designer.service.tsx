...
const ServiceComponent = defineComponent({
  props: {
    option: { type: Object as PropType<ButtonGroupDesignerServiceOption>, required: true },
  },
  setup(props) {
    ...
    const methods = {
      service: (option: ButtonGroupDesignerServiceOption) => {
        state.option = option;
      ...
      addChild: (row: ButtonGroupDesignerButton) => {
      ...
      copy: (row: ButtonGroupDesignerButton) => {
      ...
      remove: (row: ButtonGroupDesignerButton) => {
        const location = findButtonLocation(state.buttons, row);
        if (!location) return;
      ...
      move: (row: ButtonGroupDesignerButton, offset: number) => {
        const location = findButtonLocation(state.buttons, row);
        if (!location) return;
    ...
    return () => (
      <ElDialog
        ...
        title={state.option.title || '按钮组设计'}
        width="min(1280px, calc(100vw - 40px))"
        top="4vh"
        class="button-group-designer-dialog grid-designer-dialog"
        destroyOnClose={true}
      >
        {{
          default: () => (
...
export const $$buttonGroupDesigner = (() => {
  let ins: any;
  return (option: Omit<ButtonGroupDesignerServiceOption, 'onConfirm'>) => {
    if (!ins) {
    ...
    const dfd = defer<ButtonGroupDesignerResult>();
    ins.service({
      ...option,
