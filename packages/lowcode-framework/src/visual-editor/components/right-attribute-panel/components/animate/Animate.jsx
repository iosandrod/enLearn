...
export const Animate = defineComponent({
    setup() {
        ...
        const runAnimation = (animation = []) => {
            const blockRef = window.$$refs[currentBlock.value._vid];
            let animateEl = (blockRef && '$el' in blockRef ? blockRef.$el : undefined) ??
                blockRef;
        ...
        {currentBlock.value.animations?.map((item, index) => (<ElAlert key={item.value} type={'info'} style={{ marginTop: '12px' }} onClose={() => delAnimate(index)}>
            {{
                    title: () => (<div>
        ...
        // 可添加的动画列表组件
        const AnimateList = () => (<ElTabs v-model={state.activeName} stretch>
        {Object.entries(animationTabs).map(([tabKey, animationBox]) => (<ElTabPane label={animationTabs[tabKey].label} name={tabKey} key={tabKey}>
            <ElRow gutter={10}>
              {animationBox.value.map((animateItem) => (<ElCol span={8} key={animateItem.value}>
                  <div class={'animate-item'} onClick={() => addOrChangeAnimate(animateItem)} onMouseenter={() => runAnimation(animateItem)}>
                    {animateItem.label}
                  </div>
                </ElCol>))}
            </ElRow>
        ...
        return () => (<div ref={target} class={styles.animate}>
        <div v-show={!state.isAddAnimates}>
          <ElButton type={'primary'} disabled={!currentBlock.value.animations} plain icon={Plus} onClick={() => (state.isAddAnimates = true)}>
            添加动画
          </ElButton>
          <ElButton type={'primary'} disabled={!currentBlock.value.animations?.length} plain icon={CaretRight} onClick={() => runAnimation(currentBlock.value.animations)}>
            播放动画
          </ElButton>
          <AddedAnimateList />
        </div>
