...
import { ElInput, ElCheckboxGroup, ElCheckbox, ElCollapse, ElCollapseItem, ElTabs, ElTabPane, ElForm, ElIcon, } from '../../../../../common/designer-ui';
import { useVModel } from '@vueuse/core';
import { cloneDeep } from 'lodash-es';
import { Rank, CirclePlus, Remove } from '../../../../../common/remix-icons';
import { PropConfig } from '../prop-config';
import { isObject } from '../../../../../../utils/is';
import { useVisualData } from '../../../../../../hooks/useVisualData';
const DraggableView = Draggable;
const ElIconView = ElIcon;
export const CrossSortableOptionsEditor = defineComponent({
    ...
    setup(props, { emit }) {
        ...
        <ElCheckboxGroup modelValue={checkList.value} style={{ fontSize: 'inherit' }} onChange={onChange}>
          <DraggableView tag="ul" list={state.list} class="list-group" component-data={{
                tag: 'ul',
                type: 'transition-group',
                name: !state.drag ? 'flip-list' : null,
            }} handle=".handle" {...dragOptions.value} itemKey={''} onStart={() => (state.drag = true)} onEnd={() => (state.drag = false)}>
            {{
                item: ({ element, index }) => (<div class={'flex items-center justify-between'}>
                  ...
                  {isObject(element) ? (<>
                      ...
                      label:
                      <ElInput v-model={element.label} class={'my-12px mx-3px'} style={{ width: '108px' }} size="small"></ElInput>
                      value:
                      <ElInput v-model={element.value} class={'my-12px mx-3px'} style={{ width: '106px' }} size="small"></ElInput>
                    </>) : (<ElInput v-model={state.list[index]} class={'m-12px'} style={{ width: '270px' }} size="small"></ElInput>)}
                  <div class={'flex flex-col'}>
                    <ElIconView class="hover:text-blue-400 cursor-pointer" onClick={() => incrementOption(index)}>
                      <CirclePlus></CirclePlus>
                    </ElIconView>
                    <ElIconView class="hover:text-red-500 cursor-pointer" onClick={() => state.list.splice(index, 1)}>
                      <Remove></Remove>
                    </ElIconView>
                  </div>
                </div>),
        ...
        {props.showItemPropsConfig && (<ElCollapse>
            <ElCollapseItem title={'选项配置'}>
              <ElTabs type={'border-card'}>
                {state.list.map((item) => (<ElTabPane label={item.label} key={item.label}>
