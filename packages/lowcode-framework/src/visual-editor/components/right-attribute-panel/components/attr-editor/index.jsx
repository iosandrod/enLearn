...
import { applyMaterialPropFieldValue, createMaterialPropForm, createMaterialPropModel, createMaterialPropOptionSources, } from '../../../../material-prop-forms';
    ...
    setup() {
        ...
        const handleFieldChange = (payload) => {
            if (!currentBlock.value?._vid)
                return;
            applyMaterialPropFieldValue(currentBlock.value, payload.field, payload.value);
        };
        return () => (<>
        {currentBlock.value?._vid ? (<div class="material-prop-form">
            <LowCodeForm key={`${currentBlock.value._vid}-${currentBlock.value.componentKey}-${historyState.restoreVersion}`} schema={formState.value.schema} modelValue={formState.value.model} optionSources={formState.value.optionSources} onFieldChange={handleFieldChange}/>
          </div>) : (<ElEmpty description="请选择画布节点" imageSize={96}/>)}
      </>);
    },
});
