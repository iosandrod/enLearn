...
import { ElForm, ElFormItem, ElInput, ElSelect, ElOption, ElCard, ElButton, ElCascader, ElCollapse, ElCollapseItem, ElPopconfirm, } from '../../../common/designer-ui';
...
export const EventAction = defineComponent({
    setup() {
         ...
         */
        const isEdit = computed(() => currentBlock.value.actions?.some((item) => item.key === state.ruleForm.key));
        ...
        const showOperateModal = () => {
            ...
            <ElFormItem label="事件" prop={'event'} rules={[{ required: true, message: '请选择事件', trigger: 'change' }]}>
              <ElSelect v-model={state.ruleForm.event} class={'w-full'}>
                {currentBlock.value.events?.map((eventItem) => (<ElOption key={eventItem.value} label={eventItem.label} value={eventItem.value}></ElOption>))}
              </ElSelect>
            </ElFormItem>
            <ElFormItem label="事件名称" prop="name" rules={[{ required: true, message: '请输入事件名称', trigger: 'change' }]}>
              <ElInput v-model={state.ruleForm.name} placeholder={'请输入事件名称'}></ElInput>
            </ElFormItem>
            {!state.ruleForm.handle?.length && (<ElFormItem>
                <ElButton onClick={addActionHanleItem} type={'primary'}>
                  添加动作
                </ElButton>
              </ElFormItem>)}
            {state.ruleForm.handle.map((handleItem, index) => (<ElCard key={handleItem.key} shadow={'hover'} class={'mt-10px'} v-slots={{
                            header: () => (<div class={'flex justify-between'}>
                      <ElFormItem label="动作名称" prop={`handle.${index}.name`} rules={[{ required: true, message: '请输入动作名称', trigger: 'change' }]}>
                ...
                <ElFormItem label="触发的动作" prop={`handle.${index}.link`} rules={[{ required: true, message: '请选择你要触发的动作', trigger: 'change' }]}>
                  <ElCascader clearable={true} class={'w-full'} placeholder="请选择你要触发的动作" v-model={handleItem.link} options={actionOptions.value}></ElCascader>
                </ElFormItem>
              </ElCard>))}
          </ElForm>),
                onConfirm: () => {
        ...
        {currentBlock.value.actions?.map((actionItem, index) => (<ElCard key={index} class={'mt-10px'} v-slots={{
                    header: () => (<div class={'flex justify-between'}>
                  {actionItem.name}
                  <div>
            ...
            <ElCollapse v-model={state.activeNames}>
              {actionItem.handle.map((item, index) => (<ElCollapseItem title={`${index + 1}. ${item.name}`} key={item.key} name={item.key}>
                  {{
