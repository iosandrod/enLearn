...
export default defineComponent({
    ...
    setup(props) {
        ...
        const isFormBlock = () => {
            ...
            return (['form', 'lowcode-search-form', 'lowcode-edit-form'].includes(block.componentKey) ||
        ...
        watch(() => currentBlock.value.componentKey, () => {
        ...
        <div class={[styles.drawer, { [styles.isOpen]: state.isOpen }]}>
          ...
          <div class={styles.attrs}>
            <ElTabs v-model={state.activeName} type="border-card" stretch={true} class={styles.tabs}>
              ...
              {isFormBlock() ? (<ElTabPane label="规则" name="form-rule" lazy>
                  <div class={styles.panelBody}>
                    <FormRule />
                  </div>
                </ElTabPane>) : null}
              {props.showPageSetting ? (<ElTabPane label="页面设置" name="page-setting">
                  <div class={styles.panelBody}>
                    <PageSetting />
                  </div>
                </ElTabPane>) : null}
