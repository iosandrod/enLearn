 ...
 * @returns { apis, models }
 */
export const importSwaggerJson = (swagger) => {
    swagger = typeof swagger == 'string' ? JSON.parse(swagger) : swagger;
    const models = [];
    Object.entries(swagger.definitions).forEach(([name, model]) => {
    ...
    Object.entries(swagger.paths).forEach(([url]) => {
        Object.keys(swagger.paths[url]).forEach((method) => {
            ...
            const api = {
                ...
                options: {
                    url, // 请求的url
                    method: method.toLocaleUpperCase(), // 请求的方法
...
export const useImportSwaggerJsonModal = () => {
    ...
    return {
        showImportSwaggerJsonModal: () => useModal({
            title: '导入swagger JSON (支持swagger: 2.0)',
            props: {
                width: 760,
            },
            content: () => (<MonacoEditor code={''} layout={{ width: 700, height: 600 }} vid={-1} onChange={handleSchemaChange} title=""/>),
            onConfirm: () => {
                try {
