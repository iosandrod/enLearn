...
export function createMaterialPropForm(component, block) {
    const definition = getMaterialPropFormDefinition(component?.key);
    const generatedFields = definition?.extendsVisualProps === false ? [] : createFieldsFromVisualProps(component);
...
export function createMaterialPropModel(block, fields) {
    ensureDefaultValues(block, fields);
...
export function createMaterialPropOptionSources(models) {
    return {
        [visualModelsSourceKey]: cloneDeep(models),
    };
}
export function applyMaterialPropFieldValue(block, field, value) {
...
function createFieldsFromVisualProps(component) {
    return Object.entries(component?.props ?? {}).map(([propName, propConfig]) => createFieldFromVisualProp(propName, propConfig));
}
function createFieldFromVisualProp(propName, propConfig) {
    const baseField = {
        field: propName,
        target: 'props',
        path: propName,
    ...
    if (propConfig.type === VisualEditorPropsType.select) {
        return {
            ...
            component: 'lc-option-select',
            valueKind: 'raw',
            options: (propConfig.options ?? []).map((item, index) => toRawOption(item.label, item.value, String(index))),
            props: {
...
function createTableMaterialField(baseField, propConfig) {
...
function createCrossSortableMaterialField(baseField, propConfig) {
...
function createTableColumns(propConfig) {
...
function createCrossSortableColumns(defaultValue) {
    const keys = new Set();
...
function createArrayTableColumn(field, title) {
...
function getColumnDefaultValue(column) {
    if (column.component === 'vxe-switch')
        return false;
    if (column.field === 'label' || column.field === 'title')
        return `${column.title} {{index}}`;
...
function getColumnPlaceholder(field) {
    if (field === 'field')
        return 'email';
...
function inferRowKey(columns) {
    ...
    return ['field', 'code', 'name', 'key', 'value', 'label', 'title'].find((field) => fields.includes(field)) ?? '__rowKey';
}
function toSelectOption(label, value) {
    return {
        label,
        value,
    };
...
function mergeFields(generatedFields, overrideFields) {
...
function ensureDefaultValues(block, fields) {
...
function readFieldValue(block, field) {
    const target = getTargetObject(block, field.target);
    const path = field.path ?? field.field;
    const { propObj, prop } = useDotProp(target, path);
...
function getTargetObject(block, target = 'props') {
    if (target === 'block')
        return block;
    if (target === 'styles') {
        block.styles ??= {};
...
function normalizeFieldValue(field, value, currentValue) {
...
function clampLayoutSpan(value, fallback = minLayoutSpan) {
    const numeric = Number(value);
    if (!Number.isFinite(numeric))
        return fallback;
    return Math.min(layoutGridSpan, Math.max(minLayoutSpan, Math.round(numeric)));
}
function parseLayoutRatio(value, fallback = [12, 12]) {
...
function readLayoutSlotItems(value) {
...
function stringifyLayoutSlots(value) {
...
function createLayoutSlotsFromRatio(value, currentValue) {
    ...
    return spans.reduce((prev, span, index) => {
...
function toRawOption(label, rawValue, value) {
    return {
        label,
        value,
        rawValue,
