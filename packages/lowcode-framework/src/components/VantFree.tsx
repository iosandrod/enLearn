...
function emitModel(emit: (event: string, ...args: any[]) => void, value: unknown) {
    ...
    return () => {
      ...
      const control = inputSlot
        ...
        : h(type, {
            ...
            onInput: (event: Event) =>
              emitModel(emit, (event.target as HTMLInputElement | HTMLTextAreaElement).value),
      ...
      return h(
        'label',
        {
          class: ['lc-vant-free-field', fieldAttrs.class],
          style: {
            display: 'grid',
            width: '100%',
            minWidth: 0,
        ...
        [
          label
            ? h(
                ...
                [fieldAttrs.required ? h('span', { style: { color: '#dc2626' } }, '* ') : null, label],
              )
            : null,
          h('div', { style: { minWidth: 0, width: '100%' } }, control),
        ],
      );
    };
  },
  ...
  setup(_, { attrs, slots, emit }) {
    return () => {
      ...
      return h(
        ...
        {
          ...buttonAttrs,
          type: buttonAttrs['native-type'] || buttonAttrs.nativeType || 'button',
          ...
          style: {
            ...
            borderRadius: buttonAttrs.round ? '999px' : buttonAttrs.square ? '0' : '4px',
        ...
        renderDefault(slots, buttonAttrs.loading ? buttonAttrs['loading-text'] || 'Loading' : buttonAttrs.text),
      );
    };
  },
});
      ...
      h(
        'div',
        {
          ...
          style: {
            ...
            alignItems: attrs.align === 'center' ? 'center' : attrs.align === 'bottom' ? 'flex-end' : 'flex-start',
            justifyContent:
              attrs.justify === 'end'
                ? 'flex-end'
...
export const Switch: any = defineComponent({
  ...
  setup(_, { attrs, emit }) {
    return () =>
      h('input', {
        type: 'checkbox',
        checked: readModel(attrs as AnyRecord) === ((attrs as AnyRecord).activeValue ?? true),
        disabled: (attrs as AnyRecord).disabled,
        onChange: (event: Event) => {
          const checked = (event.target as HTMLInputElement).checked;
          emitModel(emit, checked ? ((attrs as AnyRecord).activeValue ?? true) : ((attrs as AnyRecord).inactiveValue ?? false));
        },
        onClick: (event: MouseEvent) => emit('click', event),
      });
  },
  ...
  setup(_, { attrs, emit }) {
    return () =>
      h('input', {
        ...
        disabled: (attrs as AnyRecord).disabled,
        style: { width: '100%' },
        onInput: (event: Event) => emitModel(emit, Number((event.target as HTMLInputElement).value)),
      });
  },
});
...
export const Rate: any = defineComponent({
  ...
  setup(_, { attrs, emit }) {
    return () => {
      ...
      return h(
        'span',
        { style: { display: 'inline-flex', gap: '4px', color: '#f59e0b' } },
...
export const Checkbox: any = defineComponent({
  ...
  setup(_, { attrs, slots }) {
    return () =>
      h('label', { style: { display: 'inline-flex', alignItems: 'center', gap: '4px', ...(attrs.style as AnyRecord) } }, [
        h('input', { type: 'checkbox', value: (attrs as AnyRecord).name }),
        renderDefault(slots),
      ]);
  },
});
...
export const Radio: any = defineComponent({
  ...
  setup(_, { attrs, slots }) {
    return () =>
      h('label', { style: { display: 'inline-flex', alignItems: 'center', gap: '4px', ...(attrs.style as AnyRecord) } }, [
        h('input', { type: 'radio', name: (attrs as AnyRecord).name, value: (attrs as AnyRecord).name }),
        renderDefault(slots),
      ]);
  },
});
  ...
  setup(_, { attrs }) {
    return () =>
      h('img', {
        ...
        style: { maxWidth: '100%', objectFit: (attrs as AnyRecord).fit || 'cover', ...(attrs.style as AnyRecord) },
      });
  },
});
...
export const Progress: any = defineComponent({
  ...
  setup(_, { attrs }) {
    return () => {
      const percentage = Math.min(100, Math.max(0, Number((attrs as AnyRecord).percentage) || 0));
      return h('div', { style: { width: '100%', height: '8px', borderRadius: '999px', background: '#e2e8f0' } }, [
        h('div', {
          style: {
            width: `${percentage}%`,
            height: '100%',
  ...
  setup(_, { attrs, slots }) {
    return () =>
      h(
        ...
        [h('span', { style: { flex: 1, borderTop: '1px solid #e2e8f0' } }), renderDefault(slots, (attrs as AnyRecord).content), h('span', { style: { flex: 1, borderTop: '1px solid #e2e8f0' } })],
      );
  },
});
  ...
  setup(_, { attrs, slots, emit }) {
    return () =>
      h(
        ...
        [
          h('button', { type: 'button', onClick: () => emit('click-left'), style: { border: 0, background: 'transparent' } }, (attrs as AnyRecord).leftText || ''),
          h('strong', { style: { textAlign: 'center' } }, renderDefault(slots, (attrs as AnyRecord).title)),
          h('button', { type: 'button', onClick: () => emit('click-right'), style: { border: 0, background: 'transparent' } }, (attrs as AnyRecord).rightText || ''),
        ],
      );
  },
});
...
export const Popup: any = defineComponent({
  ...
  inheritAttrs: false,
  setup(_, { attrs, slots }) {
    const visible = computed(() => (attrs as AnyRecord).show ?? (attrs as AnyRecord).modelValue ?? true);
    return () =>
      visible.value
        ? h(
            'div',
...
export const Picker: any = defineComponent({
  ...
  setup(_, { attrs, emit }) {
    return () =>
      h('select', {
        value: toArray((attrs as AnyRecord).columns)[(attrs as AnyRecord).defaultIndex || 0]?.value,
        style: { width: '100%' },
        onChange: (event: Event) => {
          const selected = toArray((attrs as AnyRecord).columns).find(
            (item: AnyRecord) => item.value === (event.target as HTMLSelectElement).value,
          );
          emit('change', selected);
          emit('confirm', selected);
        },
      }, toArray((attrs as AnyRecord).columns).map((item: AnyRecord) => h('option', { value: item.value }, item.label)));
  },
});
