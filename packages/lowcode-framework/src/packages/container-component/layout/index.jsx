...
const MAX_SPAN = 24;
const slotsTemp = {};
const clampSpan = (value, fallback = MIN_SPAN) => {
    const numeric = Number(value);
    if (!Number.isFinite(numeric))
        return fallback;
    return Math.min(MAX_SPAN, Math.max(MIN_SPAN, Math.round(numeric)));
};
const stringifySpans = (spans) => spans.map((span) => String(clampSpan(span))).join(':');
const parseRatio = (value, fallback = [12, 12]) => {
...
const isSlotItem = (value) => typeof value === 'object' && value !== null && !Array.isArray(value);
const sortSlotEntries = (slots) => Object.entries(slots)
...
const readSlotItems = (slots) => {
    if (!isSlotItem(slots))
        return [];
    return sortSlotEntries(slots).map(([key, item], index) => ({
        ...item,
...
const createSlotsFromSpans = (spans, previousSlots) => {
    ...
    return spans.reduce((prev, curr, index) => {
        ...
        return prev;
    }, { value: stringifySpans(spans) });
};
const createSlots = (str) => createSlotsFromSpans(parseRatio(str));
const getSlotRatioText = (slots, fallbackItems) => {
    if (isSlotItem(slots) && typeof slots.value === 'string') {
...
export default {
    ...
    render: ({ props, styles, block, custom }) => {
        ...
        const commitSlotItems = (items) => {
            const nextSlots = createSlotsFromSpans(items.map((item) => item.span), items.reduce((prev, item, index) => {
                prev[`slot${index}`] = {
                    ...item,
                    key: `slot${index}`,
        ...
        const ensureLayoutSlots = () => {
            ...
            const currentValue = isSlotItem(props.slots) ? props.slots.value : '';
        ...
        const addSlotAfter = (event, index) => {
        ...
        const startResize = (event, index) => {
            ...
            const handleMove = (moveEvent) => {
                ...
                if (!nextItems[index] || !nextItems[index + 1])
                    return;
        ...
        return () => {
          ...
          <Row ref={setRowRef} {...custom} {...rowProps} class={styleModule.vanRow}>
            {slotItems.map((spanItem, spanIndex) => {
                    slotsTemp[block._vid][`slot${spanIndex}`] = spanItem;
                    return (<Col key={spanItem.key} span={spanItem.span} class={styleModule.layoutColumn}>
                  <div class={styleModule.layoutColumnBody}>
                    <div class={styleModule.layoutColumnToolbar} onMousedown={(event) => event.stopPropagation()}>
                      ...
                      <button type="button" title="添加列" class={styleModule.layoutColumnAction} onClick={(event) => addSlotAfter(event, spanIndex)}>
                        +
                      </button>
                      <button type="button" title="删除列" class={styleModule.layoutColumnAction} disabled={slotItems.length <= 1} onClick={(event) => removeSlot(event, spanIndex)}>
                        x
                      </button>
                    </div>
