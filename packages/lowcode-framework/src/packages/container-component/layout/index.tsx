...
interface SlotItem { key: string; span: number; children: any[]; [prop: string]: any }
type LayoutSlots = Record<string, SlotItem | string | undefined> & { value?: string };
...
const stringifySpans = (spans: unknown[]) => spans.map((span) => String(clampSpan(span))).join(':');
const parseRatio = (value: unknown, fallback: number[] = [12, 12]) => {
  ...
  const spans = String(value || '').split(/[:：\s]+/).map((span) => clampSpan(span, 0)).filter((span) => span > 0).slice(0, MAX_COLUMNS);
  return spans.length ? spans : fallback;
};
const isSlotItem = (value: unknown): value is SlotItem => typeof value === 'object' && value !== null && !Array.isArray(value);
const sortSlotEntries = (slots: LayoutSlots) => Object.entries(slots)
  .filter(([key, value]) => key !== 'value' && isSlotItem(value))
  .sort(([prevKey], [nextKey]) => Number(prevKey.replace('slot', '')) - Number(nextKey.replace('slot', ''))) as [string, SlotItem][];
...
const createSlotsFromSpans = (spans: number[], previousSlots?: unknown): LayoutSlots => {
  ...
  return spans.reduce((prev, curr, index) => {
    const previousItem = previousItems[index];
    prev[`slot${index}`] = { key: `slot${index}`, span: clampSpan(curr), children: previousItem?.children || [] };
    return prev;
  }, { value: stringifySpans(spans) } as LayoutSlots);
};
const getSlotRatioText = (slots: unknown, fallbackItems: SlotItem[]) => {
  if (isSlotItem(slots) && typeof (slots as LayoutSlots).value === 'string') return String((slots as LayoutSlots).value);
...
export default {
  ...
  label: '布局容器',
  preview: () => <Row gutter="20"><Col span="8">span: 8</Col><Col span="8">span: 8</Col><Col span="8">span: 8</Col></Row>,
  render: ({ props, styles, block, custom }) => {
    ...
    const commitSlotItems = (items: SlotItem[]) => {
      const nextSlots = createSlotsFromSpans(
        ...
        items.reduce((prev, item, index) => {
          prev[`slot${index}`] = { ...item, key: `slot${index}`, children: item.children || [] };
          return prev;
        }, {} as LayoutSlots),
      );
    ...
    const ensureLayoutSlots = () => {
      ...
      const fallbackSpans = currentItems.length ? currentItems.map((item) => item.span) : parseRatio(DEFAULT_RATIO);
    ...
    const addSlotAfter = (event: MouseEvent, index: number) => {
      ...
      if (source && source.span > MIN_SPAN) { newSpan = Math.floor(source.span / 2); source.span -= newSpan; }
      else { const donor = items.find((item) => item.span > MIN_SPAN); if (donor) donor.span -= MIN_SPAN; }
      items.splice(index + 1, 0, { key: `slot${index + 1}`, span: newSpan, children: [] });
    ...
    const removeSlot = (event: MouseEvent, index: number) => {
      ...
      if (target && removed) { target.span += removed.span; target.children = [...(target.children || []), ...(removed.children || [])]; }
      commitSlotItems(items);
    };
    const startResize = (event: MouseEvent, index: number) => {
      event.stopPropagation(); event.preventDefault();
      const items = getLayoutItems();
      const left = items[index]; const right = items[index + 1];
      if (!left || !right) return;
      const unitWidth = (rowElement?.getBoundingClientRect().width || 0) / MAX_SPAN;
      if (!unitWidth) return;
      const startX = event.clientX; const startLeftSpan = left.span; const pairTotal = left.span + right.span;
      const handleMove = (moveEvent: MouseEvent) => {
        moveEvent.preventDefault();
        const delta = Math.round((moveEvent.clientX - startX) / unitWidth);
        const nextLeftSpan = Math.min(pairTotal - MIN_SPAN, Math.max(MIN_SPAN, startLeftSpan + delta));
        const nextItems = getLayoutItems();
        if (!nextItems[index] || !nextItems[index + 1]) return;
        nextItems[index].span = nextLeftSpan; nextItems[index + 1].span = pairTotal - nextLeftSpan;
    ...
    const setRowRef = (el: unknown) => { rowElement = ((el as any)?.$el || el || null) as HTMLElement | null; registerRef(el, block._vid); };
    watchEffect(() => {
      readSlotItems(props.slots).forEach((slot, index) => {
        const key = `slot${index}`;
        if (slotsTemp[block._vid][key]?.children) slot.children = slotsTemp[block._vid][key].children;
      });
    });
    return () => {
      const slotItems = readSlotItems(ensureLayoutSlots());
      const { slots: _layoutSlots, ...rowProps } = props;
      return (
        <div style={styles}>
          <Row ref={setRowRef} {...custom} {...rowProps} class={styleModule.vanRow}>
            {slotItems.map((spanItem: SlotItem, spanIndex) => {
              slotsTemp[block._vid][`slot${spanIndex}`] = spanItem;
              return (
                <Col key={spanItem.key} span={spanItem.span} class={styleModule.layoutColumn}>
                  <div class={styleModule.layoutColumnBody}>
                    <div class={styleModule.layoutColumnToolbar} onMousedown={(event) => event.stopPropagation()}>
                      <span class={styleModule.layoutColumnSpan}>span {spanItem.span}</span>
                      <button type="button" title="Add column" class={styleModule.layoutColumnAction} onClick={(event) => addSlotAfter(event, spanIndex)}>+</button>
                      <button type="button" title="Delete column" class={styleModule.layoutColumnAction} disabled={slotItems.length <= 1} onClick={(event) => removeSlot(event, spanIndex)}>x</button>
                    </div>
                    {renderSlot(slots, `slot${spanIndex}`)}
                  </div>
                  {spanIndex < slotItems.length - 1 && <span class={styleModule.layoutResizeHandle} title="拖动调整列宽" onMousedown={(event) => startResize(event, spanIndex)} />}
                </Col>
              );
            })}
          </Row>
