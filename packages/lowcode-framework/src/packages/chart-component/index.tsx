...
function buildChartOption(props: Record<string, any>, fallbackKind: ChartKind): EChartsOption {
  const optionOverride = parseJson<Record<string, unknown> | null>(props.optionJson, null);

  if (optionOverride && typeof optionOverride === 'object' && !Array.isArray(optionOverride)) {
    return optionOverride as EChartsOption;
  }
    ...
    >
      {linePath ? (
        <svg viewBox="0 0 180 64" width="180" height="64" aria-hidden="true">
          <polyline
            points="6,48 36,30 66,38 96,16 126,24 160,10"
            fill="none"
            stroke={preset.color}
            stroke-width="4"
          ...
          {preset.kind === 'area' ? (
            <polygon
              points="6,48 36,30 66,38 96,16 126,24 160,10 160,62 6,62"
              fill={preset.color}
              opacity="0.18"
            />
          ) : null}
        ...
        <svg viewBox="0 0 80 70" width="120" height="70" aria-hidden="true">
          ...
          <polygon
            points="40,14 62,30 54,54 24,56 16,28"
            fill={preset.color}
            opacity="0.24"
            stroke={preset.color}
            stroke-width="3"
