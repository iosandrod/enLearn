...
import { createEditorCrossSortableProp, createEditorInputProp, createEditorSwitchProp, createEditorColorProp, } from '../../../visual-editor/visual-editor.props';
...
export default {
    ...
    render: ({ props, block }) => {
        ...
        onMounted(() => {
            ...
            const dragArea = document.querySelector('.simulator-editor-content > .dragArea ');
            const tabbarEl = draggableEl?.querySelector('.van-tabbar');
        ...
        return () => (<Tabbar ref={(el) => registerRef(el, block._vid)} v-model={props.modelValue} {...props}>
        {props.tabs?.map((item) => {
                const itemProps = item.block?.props;
                const url = `${BASE_URL}${props.baseUrl}${itemProps.url}`.replace(/\/{2,}/g, '/');
                return (<TabbarItem name={item.value} key={item.value} {...itemProps} url={url}>
              {item.label}
            </TabbarItem>);
            })}
      </Tabbar>);
