...
import { createEditorInputProp, createEditorModelBindProp, createEditorTableProp, createEditorInputNumberProp, } from '../../../visual-editor/visual-editor.props';
const defaultFields = [
    {
        field: 'name',
        label: '名称',
...
function renderFieldRows(fields) {
    ...
    return (<div style={{
      ...
      {rows.map((field, index) => (<div key={String(field.field || index)} style={{
                display: 'grid',
        ...
        return () => {
          ...
          <Field {...props} modelValue="" name={Array.isArray(props.name) ? [...props.name].pop() : props.name} v-slots={{
                  ...
                  {designedBlocks.length && renderDesignedBlocks
                            ? renderDesignedBlocks(designedBlocks, String(props.subFormDesignerUpdatedAt || ''))
