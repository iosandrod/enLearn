...
const defaultSchema: LowCodeFormSchema = {
  fields: [
    { field: 'name', label: 'Name', component: 'vxe-input', props: { placeholder: 'Enter name' }, span: 1 },
    { field: 'remark', label: 'Remark', component: 'vxe-textarea', props: { placeholder: 'Enter remark' }, span: 1 },
  ],
  actions: [],
};
...
function createLowCodeFormProps(props: Record<string, unknown>, schema: LowCodeFormSchema): LowCodeFormProps {
  return {
...
export default {
  ...
  label: 'Sub Form',
  preview: () => (
    <div style={{ display: 'grid', width: '220px', gap: '8px' }}>
    ...
    return (
      <div style={{ ...styles, width: '100%', minWidth: 0 }}>
        {schema ? (
          ...
          <div role="alert" style={{ display: 'grid', minHeight: '48px', placeItems: 'center', color: '#b42318', fontSize: '12px' }}>
            子表单 Schema 未配置
          </div>
        )}
      </div>
