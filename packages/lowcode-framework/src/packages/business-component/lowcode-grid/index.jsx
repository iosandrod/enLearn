import { createEditorInputProp, createEditorSwitchProp, createEditorTableProp, } from '../../../visual-editor/visual-editor.props';
const previewRows = [
    ['Alice', '学生', '启用'],
    ['Bob', '老师', '停用'],
];
...
function resolveGridMinHeight(value) {
    ...
    if (typeof value === 'string') {
        ...
        if (px)
            return `${Math.max(Number(px[1]), 200)}px`;
    }
    return '200px';
}
export default {
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
        <tbody>
          {previewRows.map((row) => (<tr>
              {row.map((cell) => (<td style={{ borderBottom: '1px solid #ebeef5', padding: '6px' }}>{cell}</td>))}
            </tr>))}
        </tbody>
      </table>
    </div>),
    render({ props, styles, block }) {
        return () => {
          ...
          <div style={{ flex: '1 1 0', minHeight: 0, overflow: 'auto' }}>
            ...
            <tbody>
              {previewRows.map((row) => (<tr>
                  {columns.map((_column, index) => (<td style={{ borderBottom: '1px solid #ebeef5', padding: '8px' }}>
                      {row[index] ?? '-'}
                    </td>))}
                </tr>))}
            </tbody>
