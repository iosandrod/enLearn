...
function gridConfig(columns, height = 420) {
    return {
        border: true,
        stripe: true,
        showOverflow: true,
...
function resetFormDirective(blockId, values) {
    return {
        type: 'setFormValues',
        blockId,
        mode: 'replace',
...
function saveActions(label = '保存') {
    return [
        { code: 'submit', label, type: 'submit', status: 'primary' },
        { code: 'reset', label: '重置', type: 'reset' },
    ];
}
...
function disabledInput(field, label) {
    return {
        field,
        label,
        component: 'vxe-input',
...
function createCrudGrid(id, sourceKey, editorBlockId, columns, emptyValues, title) {
    return {
        id,
        kind: 'grid',
        sourceKey,
...
function stripDataSourceMutations(schema) {
    return Object.fromEntries(Object.entries(schema.dataSources ?? {}).map(([key, source]) => {
        const { saveMethod: _saveMethod, deleteMethod: _deleteMethod, ...rest } = source;
        return [key, rest];
    }));
}
function stripActionColumns(columns = []) {
    return columns.filter((column) => !column.slots?.default &&
        !['操作', 'Actions'].includes(String(column.title ?? '').trim()));
}
function findGridBlock(blocks, blockId) {
    for (const block of blocks) {
        if (block.kind === 'grid' && block.id === blockId)
            return block;
...
function createFilterAction(sourceKey, code, label, values, status) {
    return {
        code,
        label,
        ...(status ? { status } : {}),
...
function createRefreshAction(sourceKey) {
    return {
        code: 'refresh',
        label: '刷新',
...
function createListLayoutSchema(schema, config) {
...
const rolePermissionChildColumns = [
...
const permissionRoleChildColumns = [
...
const entityPermissionChildColumns = [
    { field: 'permission_code', title: '权限编码', minWidth: 220, fixed: 'left', showOverflow: 'tooltip' },
    { field: 'permission_name', title: '权限名称', minWidth: 180, showOverflow: 'tooltip' },
    { field: 'resource_type', title: '资源类型', width: 110, align: 'center', formatter: textFormatter },
    { field: 'resource_key', title: '资源键', minWidth: 180, showOverflow: 'tooltip', formatter: textFormatter },
...
export const roleManagementSchema = createListLayoutSchema({
    schemaVersion: 1,
    ...
    blocks: [
        ...
        createCrudGrid('role-grid', 'roles', 'role-form', [
...
export const permissionManagementSchema = createListLayoutSchema({
    schemaVersion: 1,
    ...
    blocks: [
        ...
        createCrudGrid('permission-grid', 'permissions', 'permission-form', [
...
export const routeManagementSchema = createListLayoutSchema({
    schemaVersion: 1,
...
export const entityManagementSchema = createListLayoutSchema({
    schemaVersion: 1,
    ...
    blocks: [
        ...
        createCrudGrid('entity-grid', 'entities', 'entity-form', [
...
export const permissionSystemPages = [
