...
import { isPlainRecord, readBoolean, readString, toTabsSlotKey, } from './helpers';
const converterModules = import.meta.glob('./*/index.ts', { eager: true });
const converterMap = {};
const converterList = [];
function normalizeModule(module) {
    ...
    if ('converter' in module && module.converter)
        return [module.converter];
    return [module];
}
function isConverterKey(value) {
    return Boolean(value);
}
export function registerVisualToLowCodeConverter(converter) {
    const keys = [
        converter.type,
...
export function getVisualToLowCodeConverter(block) {
    return (converterMap[block.componentKey] ??
        converterList.find((converter) => converter.match?.(block)));
}
export function convertVisualBlock(block, context) {
    const converter = getVisualToLowCodeConverter(block);
    const runtimeBlock = (converter?.toRuntimeBlock ?? converter?.convert)?.(block, context) ?? null;
    const fillRemaining = isPlainRecord(block.layout) && readBoolean(block.layout.fillRemaining, false);
...
export function convertVisualBlocks(blocks = [], dataSources) {
...
export function convertVisualOverlayBlocks(blocks = [], dataSources) {
    return convertVisualBlocks(blocks, dataSources).filter((block) => block.kind === 'modal' || block.kind === 'drawer');
}
...
function toVisualId(value, fallback) {
...
function runtimeFieldToVisualField(field) {
    ...
    const required = Array.isArray(field.rules)
        ...
        : false;
    const editableProps = field.component === 'lc-sub-form'
        ? {
            ...cloneJson(props),
            fields: Array.isArray(props.fields)
                ? props.fields.map(runtimeFieldToVisualField)
                : [],
        }
...
function runtimeColumnsToVisualColumns(columns = []) {
    return cloneJson(columns.filter((column) => !isActionColumn(column)));
}
function runtimeGridOptionsToVisualOptions(grid) {
    const { columns: _columns, ...options } = grid;
    return cloneJson(options);
}
function runtimeGridEventsToVisualRows(events, eventNames) {
...
function convertRuntimeBlockToVisual(block, context, path) {
    if (block.kind === 'buttonGroup' || block.kind === 'toolbar') {
        ...
        return createVisualBlock({
            ...
            props: {
                ...
                buttons: actions.map((action) => runtimeActionToVisualButton(action)),
            },
        });
    }
    if (block.kind === 'grid') {
        const schema = isPlainRecord(block.schema) ? block.schema : {};
        const grid = isPlainRecord(schema.grid) ? schema.grid : {};
        ...
        return createVisualBlock({
            ...
            props: {
                ...
                serviceMethod: source?.serviceMethod ?? 'listUsers',
    ...
    if (block.kind === 'searchForm' || block.kind === 'form') {
        ...
        const fields = Array.isArray(schema.fields)
            ? schema.fields.map(runtimeFieldToVisualField)
            : [];
        ...
        return createVisualBlock({
            ...
            props: {
                ...
                sourceKey,
                submitSourceKey: block.kind === 'form'
                ...
                saveMethod: source?.saveMethod ?? '',
                postDataJson: stringifyJson(source?.postData, {}),
                initialValuesJson: block.kind === 'form' ? stringifyJson(block.initialValues, {}) : undefined,
                submitText: readString(submitAction?.label, '保存'),
                resetText: readString(resetAction?.label, '重置'),
                formActions: block.kind === 'form'
                    ? actions.map((action) => runtimeActionToVisualButton(action))
                    : undefined,
                fields,
            },
        });
    ...
    if (block.kind === 'tabs') {
        ...
        const slots = tabs.reduce((result, tab, index) => {
...
export function convertLowCodeBlocksToVisualBlocks(blocks = [], dataSources = {}) {
    const context = {
        dataSources,
        convertBlocks: (children = [], path = []) => convertLowCodeBlocksToVisualBlocks(children, dataSources).map((child, index) => ({
            ...child,
...
export function convertLowCodePageSchemaToVisualEditor(schema) {
    return {
        pages: {
            '/': {
                ...
                overlays: convertLowCodeBlocksToVisualBlocks(schema.overlays ?? [], schema.dataSources ?? {}),
            },
        },
        models: [],
        actions: {
