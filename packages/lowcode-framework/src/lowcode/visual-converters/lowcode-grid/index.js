import { isDefined, isPlainRecord, normalizeColumn, normalizeRows, readBoolean, readJsonObject, readJsonArray, readString, readVisualBlockProps, toBlockId, } from '../helpers';
...
function normalizeVisualGridProps(props) {
    const legacyOptions = normalizeGridOptions(props.gridOptions);
    const gridProps = { ...legacyOptions };
...
function toRuntimeGridOptions(gridProps) {
...
function normalizeRuntimeDirectives(value) {
    const rows = Array.isArray(value)
        ...
        : typeof value === 'string'
            ? readJsonArray(value) ?? []
            : [];
    return rows.filter((item) => isPlainRecord(item) && typeof item.type === 'string' && item.type.trim().length > 0);
}
function normalizeGridEvents(value) {
    const events = {};
    const eventNames = {};
    const rows = Array.isArray(value) ? value.filter(isPlainRecord) : [];
    rows.forEach((row) => {
        const key = readString(row.key);
        if (!key || !readBoolean(row.enabled, false))
            return;
...
function normalizeGridRowActions(value) {
...
const converter = {
    ...
    toRuntimeBlock(block, context) {
        ...
        const rowKeyField = readString(rowConfig.keyField, columns.find((column) => Boolean(column.field))?.field ?? 'id');
