...
export class RuntimeBlockMapper {
  ...
  private readonly syncRuntimeGridToVisualProps = (
    ...
    visualProps.tableName = readString(
      update.changes.tableName,
      readString(targetBlock.tableName, source?.tableName ?? source?.table_name),
    );
    visualProps.viewName = readString(
      update.changes.viewName,
      readString(targetBlock.viewName, source?.viewName),
    );
