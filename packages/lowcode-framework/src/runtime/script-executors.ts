...
export type ScriptCapabilityHandler = (
...
/** Compatibility registry for non-core helpers kept for existing page scripts. */
export class ScriptCapabilityRegistry {
  private readonly handlers = new Map<LowCodeScriptCapabilityName, ScriptCapabilityHandler>();

  register(
    capabilities: LowCodeScriptCapabilityName | LowCodeScriptCapabilityName[],
    handler: ScriptCapabilityHandler,
  ) {
    const names = Array.isArray(capabilities) ? capabilities : [capabilities];
    names.forEach((name) => this.handlers.set(name, handler));
    return this;
  }
  ...
  execute(
    ...
    if (!handler) throw new Error(`脚本能力 "${request.name}" 未注册。`);
